...
/*   Updated: 2024/02/16 19:14:40 by kfortin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_mutex_struct(t_time *time, t_philo *philo)
{
	...
	{
		if (pthread_mutex_init(&philo[i].fork.fork_mutex_right, NULL) != 0)
			return (printf("error mutex initialization\n"), 1);
		i++;
	}
	i = 0;
...
int	ft_find_philo(t_time *time, t_philo *philo, int i)
{
	...
	if (philo->time->way_to_die == ONE_PHILO)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_alone, &philo[i]);
	else if (philo->time->way_to_die == EATING)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_die_eating, &philo[i]);
	else if (philo->time->way_to_die == EAT_THEN_DIE)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_eat_then_die, &philo[i]);
	else if (philo->time->way_to_die == EAT_DIE_EVEN)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_die_thinking, &philo[i]);
	else if (philo->time->way_to_die == EAT_DIE_ODD)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_live_odd, &philo[i]);
	else if (philo->time->way_to_die == SLEEPING)
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_die_sleeping, &philo[i]);
	else
		res = pthread_create(&time->philo_tid[i], NULL, (void *)&ft_routine_principale, &philo[i]);
	if (res != 0)
